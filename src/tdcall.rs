...
/// - `report_gpa` is a valid Guest Physical Address pointing to a writable buffer
///   large enough to hold a TDREPORT_STRUCT (at least 1024 bytes)
/// - `data_gpa` is a valid Guest Physical Address pointing to a readable buffer
///   containing additional data to be included in the report (typically 64 bytes)
/// - Both addresses are properly aligned (typically 64-byte aligned)
...
///
/// The caller must ensure that:
/// - `report_mac_gpa` points to a valid REPORTMACSTRUCT buffer
/// - The buffer is properly aligned and accessible
/// - The buffer contains valid cryptographic data
/// - The buffer remains valid for the duration of this call
/// - The memory is not concurrently modified by other threads
...
/// Read multiple TDX Module global-scope metadata fields.
pub fn read_sys_metadata_multiple() {
    todo!()
}

/// Read multiple TD-scope metadata fields.
pub fn read_vm_metadata_multiple() {
    todo!()
}

/// Write multiple TD-scope metadata fields.
pub fn write_vm_metadata_multiple() {
    todo!()
}

/// Read multiple VCPU-scope metadata fields.
pub fn read_vp_metadata_multiple() {
    todo!()
}

/// Write multiple VCPU-scope metadata fields.
...
/// Assign SVN values given TDSIGSTRUCTS and reference TD measurement values.
pub fn assign_svn() {
    todo!()
}
...
/// Read multiple target TD metadata fields.
...
/// Write multiple target TD metadata fields.
